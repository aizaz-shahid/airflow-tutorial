...
nohup bash /home/airflow/gcs/data/scrape_data_gcp_single_run.sh $project_id vm-dental-elite-data $service_account $gcs_bucket scrape_dental_elite_job_data.py ../data/job-url/`date '+%Y-%m-%d'`*dental-elite*.csv >  ../data/$(date "+%Y-%m-%d-%H-%M-%S-")nohup-dental-elite.out
