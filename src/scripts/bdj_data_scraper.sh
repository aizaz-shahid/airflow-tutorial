...
# nohup bash /home/airflow/gcs/data/scrape_data_gcp_single_run.sh $project_id vm-indeed-01 $service_account $gcs_bucket scrape_indeed_job_data.py ../data/job-url/`date '+%Y-%m-%d'`*dentist.csv >  ../data/$(date "+%Y-%m-%d-%H-%M-%S-")nohup-dentist-indeed.out & 
...
nohup bash /home/airflow/gcs/data/scrape_data_gcp_single_run.sh $project_id vm-bdj-data $service_account $gcs_bucket scrape_bdj_job_data.py ../data/job-url/`date '+%Y-%m-%d'`*bdj*.csv >  ../data/$(date "+%Y-%m-%d-%H-%M-%S-")nohup-bdj.out
# nohup bash /home/airflow/gcs/data/scrape_data_gcp_single_run.sh $project_id vm-dental-elite $service_account $gcs_bucket scrape_dental_elite_job_data.py ../data/job-url/`date '+%Y-%m-%d'`*dental-elite*.csv >  ../data/$(date "+%Y-%m-%d-%H-%M-%S-")nohup-dental-elite.out &
# nohup bash /home/airflow/gcs/data/scrape_data_gcp_single_run.sh $project_id vm-dental-recruit $service_account $gcs_bucket scrape_dental_recruit_job_data.py ../data/job-url/`date '+%Y-%m-%d'`*dental-recruit*.csv >  ../data/$(date "+%Y-%m-%d-%H-%M-%S-")nohup-dental-recruit.out &
# nohup bash /home/airflow/gcs/data/scrape_data_gcp_single_run.sh $project_id vm-diamond-dental $service_account $gcs_bucket scrape_diamond_dental_staff_job_data.py ../data/job-url/`date '+%Y-%m-%d'`*diamond-dental-staff*.csv >  ../data/$(date "+%Y-%m-%d-%H-%M-%S-")nohup-diamond-dental-staff.out &
# nohup bash /home/airflow/gcs/data/scrape_data_gcp_single_run.sh $project_id vm-tempdent $service_account $gcs_bucket scrape_tempdent_job_data.py ../data/job-url/`date '+%Y-%m-%d'`*tempdent*.csv >  ../data/$(date "+%Y-%m-%d-%H-%M-%S-")nohup-tempdent.out &
# nohup bash /home/airflow/gcs/data/scrape_data_gcp_single_run.sh $project_id vm-zest-dental $service_account $gcs_bucket scrape_zest_dental_job_data.py ../data/job-url/`date '+%Y-%m-%d'`*zest-dental*.csv >  ../data/$(date "+%Y-%m-%d-%H-%M-%S-")nohup-zest-dental.out &
