...
nohup bash /home/airflow/gcs/data/scrape_data_gcp_single_run.sh $project_id vm-tempdent-data $service_account $gcs_bucket scrape_tempdent_job_data.py ../data/job-url/`date '+%Y-%m-%d'`*tempdent*.csv >  ../data/$(date "+%Y-%m-%d-%H-%M-%S-")nohup-tempdent.out
