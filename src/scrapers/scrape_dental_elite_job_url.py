...
run is as:
nohup python3 scrape_dental_elite_job_url.py sudo_password > ../data/$(date "+%Y-%m-%d-%H-%M-%S-")nohup-job-url-dental-elite.out
...
def get_all_job_urls(driver):
    ...
    try:
        while True:
            jobs = driver.find_elements_by_class_name('job-more-info-btn')
            page_count += 1
