...
def getDagRunTasks():
    dag_run = DagRun.find(dag_id='dentaway_ETL',execution_date=dag.latest_execution_date)
    instances = dag_run[0].get_task_instances()
    inst = []
    for x in instances:
        if x.task_id!='get_dag_run_tasks' and x.task_id.startswith('save_dagrun')==False:
            duration=x.duration
            if x.duration==None:
                duration=0.0
            inst.append({
...
get_dag_run_tasks = PythonOperator(
    task_id='get_dag_run_tasks',
    dag=dag,
    python_callable=getDagRunTasks,
    trigger_rule='all_done'
    # on_success_callback=setSearchTerms
)
instances = json.loads(Variable.get("instances").replace("'",'"'))
save_dagrun_report=[]
...
# get_dag_run_tasks.set_upstream([t1_data,t2_data,t3_data,t4_data,t5_data,t6_data,t7_data])
t5_data >> get_dag_run_tasks
# for i,x in enumerate(instances):
get_dag_run_tasks >> save_dagrun_report